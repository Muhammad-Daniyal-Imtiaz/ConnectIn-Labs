...
export async function getUserProfile(userId: string) {
  try {
    ...
    return await getCached(`getUserProfile:${userId}:${currentUserId || "anon"}`, [CACHE_TAGS.USERS, CACHE_TAGS.PROFILES, CACHE_TAGS.CONNECTIONS, CACHE_TAGS.FOLLOWS], async () => {
...
export async function getAllUsers(limit = 3, cursor?: string, search?: string) {
  try {
    return await getCached(`getAllUsers:${limit}:${cursor || "nc"}:${search || ""}`, CACHE_TAGS.USERS, async () => {
