...
export async function getAdminPosts(limit = 20, cursor?: string) {
  try {
    ...
    const list = await db.select().from(posts).where(whereClause).orderBy(desc(posts.createdAt)).limit(limit + 1);
    const hasMore = list.length > limit;
    const items = hasMore ? list.slice(0, limit) : list;
    const nextCursor = hasMore && items.length > 0 ? items[items.length - 1].createdAt : null;

    const enriched = await Promise.all(items.map(async (p) => {
      const [likeCount] = await db.select({ value: count() }).from(postLikes).where(eq(postLikes.postId, p.id));
      return { ...p, likeCount: likeCount?.value || 0 };
    }));
...
export async function getAdminJobs(limit = 20, cursor?: string) {
  try {
    ...
    const list = await db.select().from(jobPostings).where(whereClause).orderBy(desc(jobPostings.createdAt)).limit(limit + 1);
    const hasMore = list.length > limit;
    const items = hasMore ? list.slice(0, limit) : list;
    const nextCursor = hasMore && items.length > 0 ? items[items.length - 1].createdAt : null;
    return { success: true, jobs: items, nextCursor, hasMore };
  } catch (err: any) {
    return { success: false, error: err.message };
  }
}
...
export async function getAdminCompanies(limit = 20, cursor?: string) {
  try {
    ...
    const list = await db.select().from(companyPages).where(whereClause).orderBy(desc(companyPages.createdAt)).limit(limit + 1);
    const hasMore = list.length > limit;
    const items = hasMore ? list.slice(0, limit) : list;
    const nextCursor = hasMore && items.length > 0 ? items[items.length - 1].createdAt : null;
    return { success: true, companies: items, nextCursor, hasMore };
  } catch (err: any) {
    return { success: false, error: err.message };
  }
}
...
export async function getAdminChallenges(limit = 20, cursor?: string) {
  try {
    ...
    const list = await db.select().from(challenges).where(whereClause).orderBy(desc(challenges.createdAt)).limit(limit + 1);
    const hasMore = list.length > limit;
    const items = hasMore ? list.slice(0, limit) : list;
    const nextCursor = hasMore && items.length > 0 ? items[items.length - 1].createdAt : null;

    const enriched = await Promise.all(items.map(async (c) => {
      const [teamCount] = await db.select({ value: count() }).from(challengeTeams).where(eq(challengeTeams.challengeId, c.id));
      const [subCount] = await db.select({ value: count() }).from(challengeSubmissions).where(eq(challengeSubmissions.challengeId, c.id));
      return { ...c, teamCount: teamCount?.value || 0, submissionCount: subCount?.value || 0 };
    }));
